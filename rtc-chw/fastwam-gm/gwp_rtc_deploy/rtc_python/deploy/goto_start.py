...
用法:
    python goto_start.py                          # 内置均值姿态, 速度 30%
    python goto_start.py --speed-percent 100      # 全速
    python goto_start.py --parquet episode_0.parquet  # 用某条 episode 首帧作为目标
"""
...
class Arm:
    ...
    def connect(self):
        print(f"  [{self.label}] 连接 {self.can_name} ...")
...
def main():
    ...
    p.add_argument("--enable-timeout-s", type=float, default=20.0,
                   help="等待每只机械臂 EnablePiper 成功的超时时间")
    ...
    try:
        left.connect()
        right.connect()
        left.enable(timeout=args.enable_timeout_s)
        right.enable(timeout=args.enable_timeout_s)

...
from world_action_model.utils import make_state14_from_joint_gripper

_SUBTASK_MARKER_RE = re.compile(r"\bsubtask\s*:\s*", flags=re.IGNORECASE)
_T5_LOAD_FROM_CHOICES = ("dir", "multi_dir", "pkl", "path")
_DEFAULT_T5_VARIANT_WEIGHTS = {
    "original": 0.25,
    "simple": 0.30,
    "standard": 0.25,
    "rich": 0.10,
...
_T5_PROMPT_VARIANTS = tuple(_DEFAULT_T5_VARIANT_WEIGHTS)
_T5_REQUIRED_MULTI_VARIANTS = ("simple", "standard", "rich", "paraphrase")
...
class WAMLeRobotDataset(BaseDataset):
    ...
    def __init__(
        ...
        t5_variant_weights: dict[str, float] | None = None,
        t5_variant_mode: str = "random",
        ...
        **kwargs,
        ...
        if self.t5_load_from in ("dir", "multi_dir"):
            ...
            self.t5_original_embedding_dir = str(t5_cfg.get("original_t5_embedding_dir", os.path.join(data_path, "t5_embedding")))
            self.t5_original_embedding_pattern = str(t5_cfg.get("original_t5_embedding_pattern", "episode_{episode_index:06d}.pt"))
        ...
        self.t5_variant_weights = dict(t5_variant_weights or _DEFAULT_T5_VARIANT_WEIGHTS)
        self.t5_variant_mode = str(t5_variant_mode)
        if self.t5_load_from == "multi_dir":
            invalid = set(self.t5_variant_weights) - set(_T5_PROMPT_VARIANTS)
            if invalid:
                raise ValueError(f"invalid T5 prompt variants: {sorted(invalid)}")
            if sum(float(value) for value in self.t5_variant_weights.values()) <= 0:
                raise ValueError("t5_variant_weights must have a positive total")
            if self.t5_variant_mode != "random" and self.t5_variant_mode not in _T5_PROMPT_VARIANTS:
                raise ValueError(f"invalid t5_variant_mode: {self.t5_variant_mode!r}")
    ...
    def _load_t5_artifact_from_path(self, path: str):
    ...
    def _load_t5_from_path(self, path: str):
        artifact = self._load_t5_artifact_from_path(path)
        if artifact is None:
            return None
        return self._extract_t5_embedding(artifact)

    def _load_original_t5_for_episode(self, episode_index: int):
        path = os.path.join(
            self.t5_original_embedding_dir,
            self.t5_original_embedding_pattern.format(episode_index=episode_index),
        )
        return self._load_t5_from_path(path)

    def _select_multi_t5_embedding(self, artifact, episode_index: int) -> tuple[torch.Tensor, str]:
        if not isinstance(artifact, dict) or not isinstance(artifact.get("embeddings"), dict):
            raise ValueError("multi-prompt T5 artifact must contain an 'embeddings' dictionary")
        embeddings = artifact["embeddings"]
        missing = [name for name in _T5_REQUIRED_MULTI_VARIANTS if name not in embeddings]
        if missing:
            raise ValueError(f"multi-prompt artifact is missing variants: {missing}")

        if self.t5_variant_mode == "random":
            names = list(self.t5_variant_weights)
            weights = [float(self.t5_variant_weights[name]) for name in names]
            selected = random.choices(names, weights=weights, k=1)[0]
        else:
            selected = self.t5_variant_mode
        if selected == "empty":
            template = embeddings[_T5_REQUIRED_MULTI_VARIANTS[0]]
            return template.new_zeros((0, template.shape[-1])), selected
        if selected == "original" and selected not in embeddings:
            original = self._load_original_t5_for_episode(episode_index)
            if original is not None:
                return original, selected
            return embeddings["simple"], "simple_original_missing"
        return embeddings[selected], selected
    ...
    def _get_data(self, index: int) -> dict:
        ...
        # --- PATCHED: column name compatibility for datasets that use
        #     "actions" (plural) instead of "action", and split state columns
        #     (observation.state.joint + observation.gripper_position) instead
        #     of a combined "observation.state".  These naming differences are
        #     common in v2.1-origin datasets. ---
        if "action" not in data_dict and "actions" in data_dict:
            data_dict["action"] = data_dict["actions"]
        if "observation.state" not in data_dict:
            joint = data_dict.get("observation.state.joint")
            grip = data_dict.get("observation.gripper_position")
            if joint is not None and grip is not None:
                # Legacy bug kept for reference:
                #   np.concatenate([joint, grip], axis=-1)
                # produced [L6 joints, R6 joints, L gripper, R gripper].
                # That does not match the action layout [L6, L gripper, R6, R gripper],
                # so delta action was computed between different physical quantities.
                data_dict["observation.state"] = make_state14_from_joint_gripper(joint, grip)
        # --- END PATCHED ---
    ...
    def _load_t5_embedding(self, data_dict: dict) -> None:
        ...
        if self.t5_load_from == "multi_dir":
            ...
            artifact = self._load_t5_artifact_from_path(path)
            if artifact is None:
                embedding = self._load_original_t5_for_episode(episode_index)
                if embedding is None:
                    raise FileNotFoundError(f"missing multi-prompt T5 artifact: {path}")
                data_dict[self.t5_embedding_key] = embedding
                data_dict["t5_prompt_variant"] = "original_fallback"
                return
            embedding, variant = self._select_multi_t5_embedding(artifact, episode_index)
            data_dict[self.t5_embedding_key] = embedding
            data_dict["t5_prompt_variant"] = variant

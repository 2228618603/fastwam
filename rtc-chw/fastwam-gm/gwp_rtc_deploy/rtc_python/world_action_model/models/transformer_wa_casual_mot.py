...
class CasualWorldActionTransformer_MoT(
    ...
    def _get_or_build_action_only_prefix_cache(
        ...
        for block in self.blocks:
            ...
            # 原 detach 共享 CUDA Graph 输出缓冲区，后续 replay/mark_step 可使缓存失效。
            # cached_block = {"key": block_cache["key"].detach(), "value": block_cache["value"].detach()}
            # clone 必须在 compiled forward 外，保证跨去噪步缓存有独立存储。
            cached_block = {"key": block_cache["key"].detach().clone(),
                            "value": block_cache["value"].detach().clone()}
            if "action_encoder_key" in block_cache and "action_encoder_value" in block_cache:
                ...
                cached_block["action_encoder_key"] = block_cache["action_encoder_key"].detach().clone()
                cached_block["action_encoder_value"] = block_cache["action_encoder_value"].detach().clone()
            block_caches.append(cached_block)

        cache = {
            "blocks": block_caches,
            # 原 "encoder_action_hidden_states": encoder_action_hidden_states.detach(),
            # 同样跨去噪步复用，不能依赖 CUDA Graph 临时输出的生命周期。
            "encoder_action_hidden_states": encoder_action_hidden_states.detach().clone(),

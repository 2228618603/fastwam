...
from world_action_model.models import rtc as rtc_lib
...
class WAPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    # text_encoder/tokenizer 改为可选：启用 fixed-t5-token 模式时传 None 跳过加载，
    # 直接使用预编码的 T5 embedding，省去 ~11GB 显存
    ...
    def __call__(
        ...
        rtc_prefix_full = None
        rtc_delay = int(delay)
        if rtc_delay < 0:
            raise ValueError(f"delay must be non-negative, got {rtc_delay}")
        if rtc_delay > 0 and action_prefix is None:
            raise ValueError("action_prefix is required when delay > 0")
        if action_prefix is not None and rtc_delay > 0:
            prefix = (
                action_prefix
                if action_prefix.ndim == 3
                else action_prefix.unsqueeze(0)
            )
            if prefix.shape[1] < rtc_delay:
                raise ValueError(
                    f"action_prefix has {prefix.shape[1]} steps, delay={rtc_delay}"
                )
            if prefix.shape[-1] != action.shape[-1]:
                raise ValueError(
                    f"action_prefix dim {prefix.shape[-1]} != model action dim "
                    f"{action.shape[-1]}"
                )
            if rtc_delay >= num_action_tokens:
                raise ValueError(
                    f"delay={rtc_delay} must be smaller than action chunk "
                    f"{num_action_tokens}"
                )
            rtc_prefix_mask = rtc_lib.build_prefix_mask(
                torch.tensor([rtc_delay], device=action.device), num_action_tokens
            ).expand(action.shape[0], -1)
            rtc_prefix_full = torch.zeros_like(action)
            rtc_prefix_full[:, :rtc_delay] = prefix[:, :rtc_delay].to(
            ...
            with self.progress_bar(total=len(timesteps)) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if rtc_prefix_mask is not None:
                        timestep[
                            :,
                            action_noise_t_index : action_noise_t_index + rtc_delay,
                        ] = 0
                        action = rtc_lib.clamp_prefix(
                            action, rtc_prefix_full, rtc_prefix_mask
                        )
        ...
        # scheduler.step also updates prefix positions on the final denoising step.
        if rtc_prefix_mask is not None:
            action = rtc_lib.clamp_prefix(
                action, rtc_prefix_full, rtc_prefix_mask
            )
...
# 原接口只编动作去噪；保留默认 compile_prefix=False，避免改变训练/离线调用方。
...
def compile_policy_action_blocks(policy, mode="reduce-overhead", fullgraph=False, scope="action-blocks",
    ...
    for name in ("transformer", "transformer_2"):
        ...
        if module is None:
            continue
        # 移植 gaomeng 的条件缓存编译：每次观测构建一次，跨去噪步复用。
        # 配套要求：缓存 detach().clone()、mark_step、在同一推理线程充分预热。
        if compile_prefix:
            count = 0
            for block in module.blocks:
                block.forward_prefix_cache = torch.compile(
                    block.forward_prefix_cache, mode=mode, fullgraph=fullgraph)
                count += 1
            compiled.append(f"{name}.blocks.forward_prefix_cache[{count}]")
...
def get_policy(
    ...
    if use_fixed_t5_token:
        print("use_fixed_t5_token=True: skipping T5 text_encoder load (use precomputed embedding only)")
    vae = AutoencoderKLWan.from_pretrained(base_model, subfolder="vae", torch_dtype=torch_dtype)
    transformer = CasualWorldActionTransformer_MoT.from_pretrained(checkpoint, torch_dtype=torch_dtype)
    transformer.eval()
    scheduler = FlowMatchEulerDiscreteScheduler(shift=5.0)
    if use_fixed_t5_token:
        # Skip loading text_encoder/tokenizer (~11GB VRAM). Requests must
        # provide prompt_embeds (or a fixed_t5_path must be set).
        pipe = WAPipeline.from_pretrained(
            base_model, vae=vae, transformer=transformer, scheduler=scheduler,
            text_encoder=None, tokenizer=None, torch_dtype=torch_dtype)
    else:
    ...
    model_action_dim = int(transformer.config.in_action_channels)
    if model_action_dim < 14:
        raise ValueError(
            f"checkpoint action dimension {model_action_dim} is smaller than data dimension 14"
        )
    ...
    elif use_fixed_t5_token:
        raise ValueError(
            "use_fixed_t5_token=True requires --fixed-t5-path to point to a precomputed T5 embedding "
            "(e.g. prompt_tokens/fixed_task_token.pt)"
        )
    ...
    def inference(self, data):
        ...
        norm_state = torch_F.pad(
            norm_state, (0, model_action_dim - norm_state.shape[-1]), value=0.0
        )
        ...
        if delay > 0 and action_prefix is None:
            raise ValueError("action_prefix is required when delay > 0")
        if action_prefix is not None and delay > 0:
            prefix_abs = torch.as_tensor(
                action_prefix, dtype=state.dtype, device=device
            ).reshape(-1, 14)
            if prefix_abs.shape[0] < delay:
                raise ValueError(
                    f"action_prefix has {prefix_abs.shape[0]} steps, delay={delay}"
                )
            ...
            prefix_delta = prefix_abs[:delay] - state.reshape(1, 14) * delta_mask
            norm_prefix = ((prefix_delta - delta_min) / delta_range) * 2 - 1
            norm_prefix = torch_F.pad(
                norm_prefix,
                (0, model_action_dim - norm_prefix.shape[-1]),
                value=0.0,
            ).unsqueeze(0)
...
def load_lerobot_v3_episode(root, episode_index, action_dim=14, fps_default=30):
    """Load one episode from a LeRobot v3.0 packed dataset root."""
    ...
    ep_files = sorted(
        ...
        for dirpath, _, filenames in os.walk(ep_meta_dir)
        for filename in filenames
        if filename.endswith(".parquet")
    )
    ep_df = pd.concat([pd.read_parquet(path) for path in ep_files], ignore_index=True)
    ...
    data_df = pd.read_parquet(data_path, columns=["observation.state", "action", "episode_index", "frame_index"])
    data_df = data_df[data_df["episode_index"] == episode_index].sort_values("frame_index")
    state = np.stack(data_df["observation.state"].to_numpy())[:, :action_dim].astype(np.float32)
    action = np.stack(data_df["action"].to_numpy())[:, :action_dim].astype(np.float32)

    frames_per_view = []
    for key in view_keys:
        video_chunk = int(row[f"videos/{key}/chunk_index"])
        video_file = int(row[f"videos/{key}/file_index"])
        from_ts = float(row[f"videos/{key}/from_timestamp"])
        video_path = os.path.join(root, "videos", key, f"chunk-{video_chunk:03d}", f"file-{video_file:03d}.mp4")
        start = int(round(from_ts * fps))
        frames_per_view.append([Image.fromarray(frame) for frame in _read_frames_pyav(video_path, start, length)])
...
def inference_client(
    ...
    for i in tqdm(range(0, len(front_view_images) - action_chunk, replan)):
        ...
        # Per-chunk metrics/plots use the full predicted horizon.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Open-loop replanning horizon. The model predicts a full action chunk, "
            "but only the first --replan-steps actions are executed/evaluated before "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip loading the T5 text_encoder/tokenizer (~11GB VRAM) and use only a "
            "precomputed T5 embedding via --fixed-t5-path. Requires --fixed-t5-path."
        ),
    )
    parser.add_argument("--model-dtype", default="bf16", choices=["bf16", "bfloat16", "fp16", "float16", "fp32", "float32"])
    parser.add_argument(
        "--no-compile-transformer",
        dest="compile_transformer",
        action="store_false",
        help="Disable torch.compile for cached action-only MoT block forwards.",
    )
    parser.add_argument(
        "--compile-transformer",
        dest="compile_transformer",
        action="store_true",
        help="Enable torch.compile for cached action-only MoT block forwards. Enabled by default.",
    )
    parser.add_argument(
        "--compile-scope",
        choices=["action-blocks", "action-stack"],
        default="action-blocks",
        help="Compilation scope for the server path. action-stack compiles one callable around all cached action blocks.",
    )
    parser.add_argument(
        "--compile-mode",
        default="reduce-overhead",
        help="torch.compile mode. reduce-overhead enables compiler-managed CUDA graph capture when possible.",
    )
    parser.add_argument(
        "--compile-fullgraph",
        action="store_true",
        help="Pass fullgraph=True to torch.compile for stricter graph capture checks.",
    )
    parser.set_defaults(compile_transformer=True)
...
def main(args):
    policy = get_policy(
        ...
        use_fixed_t5_token=args.use_fixed_t5_token,
    )

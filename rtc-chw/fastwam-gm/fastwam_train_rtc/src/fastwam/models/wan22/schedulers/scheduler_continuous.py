...
class WanContinuousFlowMatchScheduler:
    ...
    def add_noise(self, original_samples: torch.Tensor, noise: torch.Tensor, timestep: torch.Tensor) -> torch.Tensor:
        ...
        if sigma.ndim > original_samples.ndim:
            raise ValueError(
                f"`timestep` has more dims ({sigma.ndim}) than samples ({original_samples.ndim})."
            )
        # 按尾部补维广播，同时覆盖逐样本与逐 token 两种 timestep：
        #   [B]    + 3D action -> [B, 1, 1]        （与旧的 view(-1, 1, 1) 等价）
        #   [B]    + 5D video  -> [B, 1, 1, 1, 1]  （与旧行为等价）
        #   [B, T] + 3D action -> [B, T, 1]        （新增，RTC 的逐 token timestep 用）
        sigma = sigma.reshape(*sigma.shape, *([1] * (original_samples.ndim - sigma.ndim)))
        return (1 - sigma) * original_samples + sigma * noise

...
from .mot import MoT
from . import rtc as rtc_utils
from .rtc import RTCConfig
...
class FastWAM(torch.nn.Module):
    ...
    def __init__(
        ...
        # Training-time RTC 动作前缀条件（arXiv:2512.05964）。默认关闭 -> 与原实现逐比特一致。
        self.rtc = RTCConfig.from_dict(rtc)
        logger.info("FastWAM action conditioning: %s", self.rtc.describe())
    ...
    def _build_action_flow_inputs(self, action: torch.Tensor):
        """Sample the action-branch flow-matching inputs, optionally with an RTC action prefix.

        Returns `(noisy_action, timestep_for_model, target_action, base_timestep, prefix_mask)`.

        `self.rtc.enabled=False` 时走与原实现完全相同的数值路径，`prefix_mask` 为 None。

        开启后：前缀位置的 timestep 置 0（本仓库 σ=0 = 干净数据），于是 `add_noise` 自动给出
        `1·A + 0·ε = A` —— **干净前缀是自动涌现的，不需要任何特判**（论文改动 2）。
        """
        batch_size, action_horizon = action.shape[0], action.shape[1]
        noise_action = torch.randn_like(action)
        base_timestep = self.train_action_scheduler.sample_training_t(
            batch_size=batch_size,
            device=self.device,
            dtype=action.dtype,
        )

        timestep_action = base_timestep
        prefix_mask = None
        if self.rtc.enabled:
            rtc_utils.validate(self.rtc, action_horizon=action_horizon)
            delay = rtc_utils.sample_delay(batch_size, self.rtc, device=self.device)
            prefix_mask = rtc_utils.build_prefix_mask(delay, action_horizon)
            timestep_action = rtc_utils.apply_prefix_timestep(base_timestep, prefix_mask)

        noisy_action = self.train_action_scheduler.add_noise(action, noise_action, timestep_action)
        target_action = self.train_action_scheduler.training_target(
            action, noise_action, timestep_action
        )
        if prefix_mask is not None and self.rtc.prefix_jitter_std > 0.0:
            noisy_action = rtc_utils.jitter_prefix(
                noisy_action, prefix_mask, self.rtc.prefix_jitter_std
            )
        return noisy_action, timestep_action, target_action, base_timestep, prefix_mask
    ...
    def _compute_action_loss(
        ...
        """Masked, timestep-weighted action flow-matching loss.

        RTC 开启时把前缀位置从 loss 中剔除（论文改动 3）：前缀输入已经是干净真实动作，
        那里的「去噪」任务是平凡的，继续算 loss 只会浪费梯度、并注入一个推理时永不执行的任务。
        归一化必须用 `sum / sum(mask)` 而非 `mean`，否则 d 大的样本会因有效 token 少而被稀释
        （论文解读 §5.1）。
        """
        ...
        if action_is_pad is None and prefix_mask is None:
            # 保留原有分支：mean 与 sum/sum 在浮点上未必逐比特相同，这里确保零回归。
            action_loss_per_sample = action_loss_token.mean(dim=1)
        else:
            valid = torch.ones_like(action_loss_token, dtype=torch.bool)
            if action_is_pad is not None:
                valid = valid & ~action_is_pad
            if prefix_mask is not None:
                valid = valid & ~prefix_mask
            valid_f = valid.to(device=action_loss_token.device, dtype=action_loss_token.dtype)
            valid_sum = valid_f.sum(dim=1).clamp(min=1.0)
            action_loss_per_sample = (action_loss_token * valid_f).sum(dim=1) / valid_sum

        # 权重用逐样本的 base timestep（`[B]`），不用逐 token 版本：后缀 token 共享同一个采样
        # τ，掩码之后二者等价，而用 `[B]` 让 RTC 关闭时的语义与原实现完全不变。
        action_weight = self.train_action_scheduler.training_weight(base_timestep).to(
    ...
    def training_loss(self, sample, tiled: bool = False):
        ...
        (
            ...
            timestep_action,
            target_action,
            base_timestep_action,
            action_prefix_mask,
        ) = self._build_action_flow_inputs(action)
        ...
        loss_action = self._compute_action_loss(
            ...
            action_is_pad=action_is_pad,
            base_timestep=base_timestep_action,
            prefix_mask=action_prefix_mask,
        )
        ...
        if action_prefix_mask is not None:
            # 监控采样到的延迟分布是否正常（uniform[0,max_delay) 下期望 (max_delay-1)/2）。
            loss_dict["rtc_mean_delay"] = float(
                action_prefix_mask.sum(dim=1).float().mean().item()
            )
    ...
    def _compiled(self, name: str, fn, mode: str):
        """Cache `torch.compile(fn)` per (name, mode).

        mode 进缓存键：改了 mode 会重新编译，而不是静默返回上一次用别的 mode 编出来的版本。
        """
        attr = f"_compiled__{name}__{mode.replace('-', '_')}"
        if not hasattr(self, attr):
            setattr(self, attr, torch.compile(fn, mode=mode, fullgraph=True))
        return getattr(self, attr)

    @staticmethod
    def _mode_uses_cudagraphs(mode: str) -> bool:
        """这些 mode 会启用 Inductor 的 CUDA Graph Trees。

        ⚠️ CUDA Graph Trees 的状态存在 `threading.local()` 里，且只在 import
        `torch._inductor.cudagraph_trees` 的那个线程上初始化（该模块第 277-292 行是顶层语句）。
        因此**从其它线程调用编译后的函数会 AssertionError**。

        这正是异步部署（`deploy_real.py` 的 `run_async` 把推理丢进 ThreadPoolExecutor）
        必须用 `mode="default"` 的原因：sync 模式、dryrun、offline_eval 都在主线程，
        所以以前一直没暴露这个问题。代价是失去 CUDA Graphs 那部分加速（本路径的主要收益来源，
        因为它只有 32 个 action token、瓶颈在 kernel launch 而非算力）。
        """
    ...
    def _prepare_action_prefix(
        ...
        """Validate an RTC action prefix and pad it to `[1, H, D]`.

        Returns `(prefix_full, prefix_mask)`, or `(None, None)` when there is nothing to condition
        on (`delay == 0`) — in that case callers keep the original per-sample 1-D timestep path so
        that existing evaluation stays numerically identical.

        `action_prefix` 接受 `[L, D]` 或 `[1, L, D]`，其中 `L >= delay`，只取前 `delay` 步。
        这样部署时可以直接把上一个完整 chunk 传进来，无需调用方切片。
        """
        delay = int(delay)
        if delay < 0:
        ...
        if delay >= action_horizon:
            raise ValueError(
                f"`delay` ({delay}) must be < `action_horizon` ({action_horizon}); "
                "otherwise there is no postfix left to generate."
            )
        if not self.rtc.enabled:
            logger.warning(
                "`delay=%d` was requested but this model was built with `rtc.enabled=false`; "
                "it was never trained on action-prefix conditioning, so results will be poor.",
                delay,
            )
        if action_prefix is None:
            raise ValueError(f"`action_prefix` is required when `delay` > 0 (got delay={delay}).")
        if action_prefix.ndim == 2:
            action_prefix = action_prefix.unsqueeze(0)
        if action_prefix.ndim != 3 or action_prefix.shape[0] != 1:
            raise ValueError(
                f"`action_prefix` must be [L, D] or [1, L, D], got {tuple(action_prefix.shape)}"
            )
        if action_prefix.shape[1] < delay:
            raise ValueError(
                f"`action_prefix` has only {action_prefix.shape[1]} steps but `delay`={delay}."
            )
        if action_prefix.shape[2] != self.action_expert.action_dim:
            raise ValueError(
                f"`action_prefix` last dim must be {self.action_expert.action_dim}, "
                f"got {action_prefix.shape[2]}"
            )
        ...
        prefix_full[:, :delay] = action_prefix[:, :delay].to(
            device=self.device, dtype=self.torch_dtype
        )
        prefix_mask = rtc_utils.build_prefix_mask(
            torch.tensor([delay], device=self.device, dtype=torch.int64), action_horizon
        )
    ...
    def infer_joint(
        ...
        self.eval()
        prefix_full, prefix_mask = self._prepare_action_prefix(
            action_prefix=action_prefix, delay=delay, action_horizon=action_horizon
        )
        if test_action_with_infer_action:
            ...
            action_only_out = self.infer_action(
                ...
                action_prefix=None if action_prefix is None else action_prefix.clone(),
                delay=delay,
            )["action"]
        ...
        if compile_action_infer:
            ...
            joint_denoise_core = self._compiled(
                "joint_denoise_core", self._joint_denoise_core, compile_mode
            )
        else:
            joint_denoise_core = self._joint_denoise_core
        use_cudagraphs = compile_action_infer and self._mode_uses_cudagraphs(compile_mode)
        ...
        for step_t_video, step_delta_video, step_t_action, step_delta_action in zip(
            ...
            if prefix_mask is not None:
                latents_action = rtc_utils.clamp_prefix(latents_action, prefix_full, prefix_mask)
                timestep_action = rtc_utils.apply_prefix_timestep_scalar(
                    timestep_action, prefix_mask
                )
    ...
    def infer_action(
        ...
        use_cudagraphs = compile_action_infer and self._mode_uses_cudagraphs(compile_mode)
        if compile_action_infer:
            prefill_video_cache = self._compiled(
                "prefill_video_cache", self.mot.prefill_video_cache_tensor, compile_mode
            )
            denoise_action_with_video_cache = self._compiled(
                "denoise_action_with_video_cache",
        ...
        if use_cudagraphs:
            # CUDA Graphs 会复用图内缓冲区，replay 时把上一次的 KV cache 覆写掉，必须 clone。
            # 非 cudagraph 的 mode（如 "default"）返回的是普通张量，不需要这次拷贝。
        ...
        for step_t_action, step_delta_action in zip(infer_timesteps_action, infer_deltas_action):
            ...
            timestep_action = step_t_action.unsqueeze(0).to(dtype=latents_action.dtype, device=self.device)
            if prefix_mask is not None:
                # ★ clamp 必须在模型调用之前，保证模型每步看到的前缀都是干净真实值，
                #   与训练时的输入分布严格一致（论文 §5.2）。
                latents_action = rtc_utils.clamp_prefix(latents_action, prefix_full, prefix_mask)
                # ★ 前缀位置 timestep 置 0（本仓库 σ=0 = 干净数据），与训练时一致。
                #   形状恒为 [1, H]，delay 只改数值不改形状 -> 不触发 torch.compile 重编译。
                timestep_action = rtc_utils.apply_prefix_timestep_scalar(
                    timestep_action, prefix_mask
                )
            ...
            # 循环里只有 forward，没有 VJP —— 这就是相对 inference-time RTC 的全部性能收益来源。
            latents_action = self.infer_action_scheduler.step(pred_action, step_delta_action, latents_action)

        if prefix_mask is not None:
            # 最后一次 step 也更新了前缀位置，返回前必须再 clamp 一次，否则返回值的前缀是「脏」的
            # （论文解读 §5.2 第 3 点：重实现时误用返回值前缀是经典坑）。
            latents_action = rtc_utils.clamp_prefix(latents_action, prefix_full, prefix_mask)

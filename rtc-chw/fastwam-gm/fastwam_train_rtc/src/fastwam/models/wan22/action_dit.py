...
class ActionDiT(nn.Module):
    ...
    def _embed_timestep(
        ...
        """Embed a per-sample `[B]` or per-token `[B, T]` flow-matching timestep.

        `[B]`    -> t `[B, C]`,    t_mod `[B, 6, C]`
        `[B, T]` -> t `[B, T, C]`, t_mod `[B, T, 6, C]`

        逐 token 分支是 training-time RTC 的动作前缀条件（arXiv:2512.05964 的改动 1）：
        前缀位置的 timestep 置 0（本仓库约定 sigma=0 是干净数据），模型即知该 token 已确定。
        与 video expert 把第一帧 timestep 置 0 的做法同源（wan_video_dit.py `pre_dit`），
        下游 `DiTBlock` / `MoT._split_modulation` 已按 `len(t_mod.shape) == 4` 支持逐 token 调制。
        本方法不引入任何新参数——`time_embedding` / `time_projection` 完全共享。
        """
        if timestep.ndim == 1:
            t = self.time_embedding(sinusoidal_embedding_1d(self.freq_dim, timestep))
            return t, self.time_projection(t).unflatten(1, (6, self.hidden_dim))
        if timestep.ndim != 2:
            raise ValueError(
                f"`timestep` must be 1D [B] or 2D [B, T], got shape {tuple(timestep.shape)}"
            )
        if tuple(timestep.shape) != (batch_size, seq_len):
            raise ValueError(
                "Per-token `timestep` shape must be [batch_size, action_seq_len], "
                f"expected {(batch_size, seq_len)}, got {tuple(timestep.shape)}"
            )
        token_emb = sinusoidal_embedding_1d(self.freq_dim, timestep.reshape(-1))
        t = self.time_embedding(token_emb).reshape(batch_size, seq_len, self.hidden_dim)
        return t, self.time_projection(t).unflatten(2, (6, self.hidden_dim))
    ...
    def pre_dit(
        ...
        t, t_mod = self._embed_timestep(timestep, batch_size=batch_size, seq_len=seq_len)
    ...
    def prepare(
        ...
        `timestep` may be per-sample `[B]` or per-token `[B, action_seq_len]`; the latter is the
        training-time RTC action-prefix condition (see `_embed_timestep`).
        """
        seq_len = action_tokens.shape[1]
        t, t_mod = self._embed_timestep(
            timestep, batch_size=action_tokens.shape[0], seq_len=seq_len
        )
        tokens = self.action_encoder(action_tokens)
        context_emb = self.text_embedding(context)
        context_attn_mask = context_mask.unsqueeze(1).expand(-1, seq_len, -1)

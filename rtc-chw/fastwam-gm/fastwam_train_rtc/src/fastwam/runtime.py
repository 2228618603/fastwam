...
def create_fastwam(
    ...
    # Training-time RTC 动作前缀条件（arXiv:2512.05964）。None / {} -> 关闭。
    if isinstance(rtc, DictConfig):
        rtc = OmegaConf.to_container(rtc, resolve=True)
    if rtc is None:
        rtc = {}
    if not isinstance(rtc, dict):
        raise ValueError(f"`rtc` must be dict-like, got {type(rtc)}")

...
class BaseLerobotDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        seed: int = 42,
        # Explicit episode selection. When given, it fully replaces the
        # `val_set_proportion` split (offline evaluation on a named subset).
        ...
        self.selected_episodes = list(episodes) if episodes is not None else None
        ...
        if self.selected_episodes is not None:
            # Explicit selection wins over the proportional split. Same list for every
            # dataset dir, so bounds are checked against each one's episode count.
            for meta in metas:
                bad = [e for e in self.selected_episodes if not 0 <= e < meta.total_episodes]
                if bad:
                    raise ValueError(
                        f"`episodes` {bad} out of range for {meta.repo_id} "
                        f"(has {meta.total_episodes} episodes)."
                    )
                episode_map.update({meta.repo_id: list(self.selected_episodes)})
        elif val_set_proportion < 1e-6:
            for meta in metas:
                episode_map.update({meta.repo_id: list(range(meta.total_episodes))})
        else:
            for meta in metas:
                ...
                if self.is_training_set:
                    episode_map.update({meta.repo_id: [episode_indices[i] for i in range(split_idx)]})
                else:
                    episode_map.update({meta.repo_id: [episode_indices[i] for i in range(split_idx, meta.total_episodes)]})

        # Resolved selection, in the order the frames are concatenated. Offline eval
        # reads this to attribute each frame index back to its source episode.
        self.episode_ids = episode_map

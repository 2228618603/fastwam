...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        # warmup 长度。原实现把比例硬编码为 0.05，总步数一大 warmup 就过长
        # （77,690 步 x 5% = 3,884 步 ≈ 2.1 小时，而实测 250 步已足够稳定）。
        # 改成可配置，默认仍是 0.05，不改变既有 LIBERO / RoboTwin 任务的行为：
        #   warmup_steps 非空 -> 直接用该绝对步数（优先级更高）
        #   否则               -> warmup_ratio x 总步数
        warmup_steps_cfg = cfg.get("warmup_steps", None)
        self.warmup_steps_override = (
            int(warmup_steps_cfg) if warmup_steps_cfg is not None else None
        )
        ...
        if self.warmup_steps_override is not None:
            warmup_steps = self.warmup_steps_override
            warmup_src = "warmup_steps (绝对值)"
        else:
            warmup_steps = int(total_train_steps * self.warmup_ratio)
            warmup_src = f"warmup_ratio={self.warmup_ratio}"
        warmup_steps = max(0, min(warmup_steps, total_train_steps - 1))
        logger.info(
            "LR schedule: type=%s total_steps=%d warmup_steps=%d (%.2f%%, 来自 %s) peak_lr=%.2e min_lr=%.2e",
            cfg.lr_scheduler_type,
            total_train_steps,
            warmup_steps,
            100.0 * warmup_steps / max(total_train_steps, 1),
            warmup_src,
            self.learning_rate,
        ...
        # ⚠️ 热启动权重必须在 accelerator.prepare() **之前**加载。
        #
        # DeepSpeed ZeRO-1/2 在引擎初始化时会把 bf16 参数 flatten 后 clone 成一份 fp32
        # master 副本（`single_partition_of_fp32_groups`），之后每个 optimizer step 结束
        # 都用 master 覆盖回 bf16 参数。所以在 prepare() **之后**才 load_state_dict 的话，
        # 权重只在第 1 次前向里生效，第 1 个 step 就被 master 副本**静默还原**。
        #
        # 这个顺序错误曾让三次热启动实验（ab_B RoboTwin / ab_C1 ab_C2 GWP-0.5）全部退化成
        # baseline 的复现：ab_B 加载的是训练完整的 FastWAM，step 10 的 loss_action 却是
        # 1.9475，与随机初始化的 ab_A(1.9514) 只差 0.2%。当时被误判为「真实负迁移」。
        #
        # 完整状态恢复（resume 指向**目录**）仍然必须在 prepare() 之后，
        # 因为它走 accelerator.load_state()，需要 DeepSpeed 引擎已经存在。
    ...
    #: 热启动守卫抽查的参数名（video / action 两侧各取几个,覆盖 mot 的两个 mixture）
    _WARMSTART_PROBE_KEYS = (
        "mot.mixtures.video.blocks.0.self_attn.q.weight",
        "mot.mixtures.video.blocks.15.ffn.0.weight",
        "mot.mixtures.action.blocks.0.self_attn.q.weight",
        "mot.mixtures.action.blocks.15.ffn.0.weight",
    )

    #: 守卫阈值。1 个 step 的合法位移量级是 lr*grad ~ 1e-5;
    #: 被 fp32 master 还原成另一份初始化则是 1e-1 量级。两者相隔 4 个数量级,
    #: 取 1e-2 既不会被正常训练触发,也不可能漏掉还原。
    _WARMSTART_REL_TOL = 1e-2

    def _load_weight_checkpoint_before_prepare(self):
        """在 prepare() 之前加载「只含权重」的热启动 checkpoint(resume 指向文件时)。

        同时记录几个参数的快照,供 `_verify_warmstart_survived()` 在第 1 个 optimizer
        step 之后核对 —— 防止将来任何原因（换 ZeRO stage、换 accelerate 版本、有人把
        调用顺序挪回去）再次让热启动静默失效。
        """
        self._warmstart_probe: dict[str, torch.Tensor] | None = None
        ...
        if resume_path.is_dir():
            return                      # 完整状态恢复,由 _resume_or_load_checkpoint 处理
        if not resume_path.exists():
            raise FileNotFoundError(f"Resume checkpoint not found: {resume}")

        logger.info("Loading weight checkpoint only (before DeepSpeed init): %s", resume)
        self.model.load_checkpoint(str(resume_path), optimizer=None)

        # 用 get_parameter() 按模块路径取,而不是查 named_parameters()。
        # FastWAM 同时持有 self.mot 和 self.video_expert/self.action_expert 指向**同一批**
        # 模块,named_parameters() 默认去重(remove_duplicate=True),这批参数到底报成
        # `mot.mixtures.video.*` 还是 `video_expert.*` 取决于属性注册顺序,不可靠。
        probe = {}
        for key in self._WARMSTART_PROBE_KEYS:
            try:
                probe[key] = self.model.get_parameter(key).detach().float().cpu().clone()
            except AttributeError:
                continue
        if not probe:
            logger.warning(
                "Warm-start guard disabled: none of the probe keys resolved against the model. "
                "Probe keys may be stale relative to the current module layout: %s",
                list(self._WARMSTART_PROBE_KEYS),
            )
            return
        self._warmstart_probe = probe
        logger.info("Warm-start guard armed on %d probe tensors.", len(probe))

    def _verify_warmstart_survived(self):
        """第 1 个 optimizer step 之后核对热启动权重是否还在。只跑一次。"""
        if self._warmstart_checked or self._warmstart_probe is None:
            return
        self._warmstart_checked = True

        model = self.accelerator.unwrap_model(self.model)
        worst_key, worst_rel = None, 0.0
        for key, ref in self._warmstart_probe.items():
            live = model.get_parameter(key).detach().float().cpu()
            denom = float(ref.norm())
            if denom == 0.0:
                continue
            rel = float((live - ref).norm()) / denom
            if rel > worst_rel:
                worst_key, worst_rel = key, rel

        if worst_rel > self._WARMSTART_REL_TOL:
            raise RuntimeError(
                f"热启动权重在第 1 个 optimizer step 后被覆盖了:{worst_key} 相对变化 "
                f"{worst_rel:.4f} > 阈值 {self._WARMSTART_REL_TOL}。\n"
                "这通常意味着权重加载又跑到了 accelerator.prepare() 之后 —— DeepSpeed 的 "
                "fp32 master 副本会在每个 step 把 bf16 参数覆盖回引擎初始化时的值。\n"
                "参见 Trainer.__init__ 里 _load_weight_checkpoint_before_prepare 的注释。"
            )
        logger.info(
            "Warm-start guard passed: max relative drift after first step = %.2e "
            "(tolerance %.0e). 热启动权重确认生效。",
            worst_rel, self._WARMSTART_REL_TOL,
        )
    ...
    def _resume_or_load_checkpoint(self):
        ...
        if resume_path.is_dir():
            ...
            return
        # 文件形式的权重已在 prepare() 之前加载完毕（见
        # _load_weight_checkpoint_before_prepare）；这里只做提示，不再重复加载。
        logger.warning(
            "Loaded .pt weights only (before DeepSpeed init); "
        ...
        while self.global_step < self.max_steps:
            ...
            with self.accelerator.accumulate(self.model):
                ...
                if self.accelerator.sync_gradients:
                    ...
                    self.optimizer.zero_grad(set_to_none=True)
                    self.global_step += 1
                    self._verify_warmstart_survived()
                    global_loss = float(

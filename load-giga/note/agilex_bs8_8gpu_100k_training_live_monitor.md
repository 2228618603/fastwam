# AgileX Giga Init BS8 8GPU 100K 训练和实时 Loss 曲线

本文档用于启动 8 卡、单卡 batch size 8、无梯度累积、100000 step 的 FastWAM AgileX 训练，并在文件夹中实时维护 loss 曲线图片和动图。

## 当前实测结论

- 使用 GPU：`0,1,2,3,4,5,6,7`
- 单卡 batch size：`8`
- 全局 batch size：`8 * 8 = 64`
- 梯度累积：`1`
- 总步数：`100000`
- 保存间隔：每 `5000` step 保存一次 checkpoint
- `bs=20` 已 OOM。
- `8 卡 bs16` 已复测：可到 step 1，但随后在 `mot._split_modulation` 处 OOM。
- 当前使用 `8 卡 bs8` 作为长跑配置。
...
```bash
tmux attach -t fastwam_bs8_8gpu_100k
```
...
监控脚本优先解析训练 log；如果 `/mnt/data` 上的 log 临时不可读或解析不到 loss 点，会自动回退到 `fastwam_bs8_8gpu_100k:train` 的 tmux pane 最近输出。可在 `status.json` 里看 `source` 字段确认当前数据来源。
...
```bash
tmux kill-session -t fastwam_bs8_8gpu_100k
```
...
```bash
SESSION=fastwam_bs8_8gpu_run2 bash load-giga/code/06_start_bs8_8gpu_training_tmux.sh
```

换输出目录：

```bash
RUN_DIR=/mnt/data/chw/fastwam/runs/agilex_empty_box_giga_init_bs8_8gpu_100k_run2 \
bash load-giga/code/06_start_bs8_8gpu_training_tmux.sh
```

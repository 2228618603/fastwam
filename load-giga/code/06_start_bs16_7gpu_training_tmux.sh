...
SESSION=${SESSION:-fastwam_bs16_7gpu}
GPUS=${GPUS:-1,2,3,4,5,6,7}
NUM_PROCESSES=${NUM_PROCESSES:-7}
RUN_DIR=${RUN_DIR:-/mnt/data/chw/fastwam/runs/agilex_empty_box_giga_init_bs16_5k}
LIVE_DIR=${LIVE_DIR:-${ROOT}/load-giga/code/live}
LOG_DIR=${LOG_DIR:-/mnt/data/chw/fastwam/logs/giga_to_fastwam}
LOG_FILE=${LOG_FILE:-${LOG_DIR}/agilex_empty_box_giga_init_bs16_5k.log}

mkdir -p "${RUN_DIR}" "${LIVE_DIR}" "${LOG_DIR}"
...
cat > "${RUN_DIR}/launch_train.sh" <<EOF
#!/usr/bin/env bash
set -euo pipefail
cd "${ROOT}"
...
cat > "${RUN_DIR}/launch_plot.sh" <<EOF
#!/usr/bin/env bash
set -euo pipefail
cd "${ROOT}"
source ~/miniconda3/etc/profile.d/conda.sh
...
python load-giga/code/07_live_loss_plot.py \\
  ...
  --gif-tail 300
EOF
chmod +x "${RUN_DIR}/launch_plot.sh"

tmux new-session -d -s "${SESSION}" -n train "bash '${RUN_DIR}/launch_train.sh'; echo; echo '[train exited]'; exec bash"
tmux new-window -t "${SESSION}" -n plot "bash '${RUN_DIR}/launch_plot.sh'; echo; echo '[plot exited]'; exec bash"
